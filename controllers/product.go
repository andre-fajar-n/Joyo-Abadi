...
func (p *product) ListProducts(c *fiber.Ctx) error {
	...
	if err := p.db.Preload("User").Find(&products).Error; err != nil {
...
func (p *product) RenderCreateProduct(c *fiber.Ctx) error {
...
func (p *product) RenderEditProduct(c *fiber.Ctx) error {
	// Get the current user ID
	userID := utils.GetLocalUserID(c)
...
func (p *product) DeleteProduct(c *fiber.Ctx) error {
	...
	var count int64
	if err := p.db.Model(&models.Transaction{}).Where("product_id = ?", id).Count(&count).Error; err != nil {
...
func (p *product) GetProductDetail(c *fiber.Ctx) error {
	...
	var product models.Product
	if err := p.db.Preload("User").First(&product, id).Error; err != nil {
		utils.Log.WithError(err).Warn("Product not found")
		return c.Redirect("/products")
	}
